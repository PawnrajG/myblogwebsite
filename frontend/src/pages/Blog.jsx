  ...
  if (!neededBlog) {
    return (
      <div className="text-center my-10">
        <p>Blog not found or failed to load the content. Please try again later.</p>
      </div>
    );
  }
  ...
  return (
    <>
          <div className='mb-16 mx-auto  md:w-[70%] px-4 md:px-1' key={blogId}>
            <h1 className='text-center text-2xl sm:text-4xl font-bold mt-10 mb-16 bg-base-100'>Blog for you</h1>
            <h1 className='text-white sm:text-4xl text-2xl font-bold my-7'>{neededBlog.title}</h1>
            <div className='border-2 border-slate-50 rounded-lg overflow-hidden mb-7'>
                <img src={neededBlog.imageUrl} className='lg:h-96 md:h-80 w-full object-cover object-center' alt="blog image"/>
            </div>                      
            <p className='h-auto p-2 text-sm md:text-[15px] mx-auto leading-relaxed text-justify text-md mb-4 text-slate-100'>{neededBlog.content}</p>
            <div className="text-gray-400 text-[13px] flex flex-col md:flex-row justify-between mx-2 mb-3">
              <p className="md:text-lg bg-base-100 text-center font-medium">Author : {neededBlog.author}</p>
              <p className="text-center text-sm pt-2 font-normal">Published on : {dayjs(neededBlog.createdAt).format("YYYY-MM-DD HH:mm:ss")}</p>
            </div>
          </div>
